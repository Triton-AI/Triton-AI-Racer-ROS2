...
namespace tritonai
{
    namespace interface
    {
        ...
        void JsTaiInterfaceNode::jsCallback(const Joy::SharedPtr joy)
        {
            ...
            ctl_msg.stamp = now();
            ctl_msg.throttle.throttle = map_paddel(joy->axes[js_config_.axis_trigger_throttle], js_config_.throttle_flipped);
            ctl_msg.brake.brake = map_paddel(joy->axes[js_config_.axis_trigger_brake], js_config_.brake_flipped);
            ctl_msg.steering_openloop.steer = map_steer(joy->axes[js_config_.axis_stick_steering], js_config_.steering_flipped);
